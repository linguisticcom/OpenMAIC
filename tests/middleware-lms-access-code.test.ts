...
describe('deployment access-code proxy for LMS routes', () => {
  ...
  it('lets LMS APIs reach their route-level session and access-code guards', async () => {
    ...
    expectNext(await proxy(request('/api/organization-auth/login')));
    expectNext(await proxy(request('/api/organization/courses')));
    expectNext(await proxy(request('/api/course-access/validate')));
    expectNext(await proxy(request('/api/validate-course-access')));
    expectNext(await proxy(request('/api/admin/organizations')));
    expectNext(await proxy(request('/api/classroom?id=classroom-cloud')));
    expectNext(await proxy(request('/api/classroom-media/classroom-cloud/media/cover.png')));
  });

  it('keeps classroom writes behind the deployment access-code gate', async () => {
    process.env.ACCESS_CODE = 'site-wide-code';

    const response = await proxy(request('/api/classroom', 'POST'));
